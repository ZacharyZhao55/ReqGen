...
def cand_dict_to_list(dict):
    cand_req = []
    for key,value in dict.items():
        cand_req.append(value)
    return cand_req

def req_structurize(tokenizer,req_list):
    cands_req_list = []
    for r in req_list:
        output_buf = tokenizer.convert_ids_to_tokens(r)
        output_tokens = []
        for t in output_buf:
            if t in ("[SEP]", "[PAD]"):
                break
            output_tokens.append(t)
        cands_req = ' '.join(detokenize(output_tokens))
        if '.' not in cands_req[-3:]:
            cands_req += '.'
        cands_req_list.append(cands_req)
    print('cands_req_list',cands_req_list)
...
def req_stru_eval(batch_n):
    gen_req_stru_path = 'unilm/src/crf_project/gannt/external/test_gannt.json'
    req_stru_path = 'uav_dataset/uav-annonation/k_folk/uav.structure.test.json'
    ...
    ref_re2st_list = RS4RE.json2dict(ref_json)

    stru_score = []
    for i in gen_re2st_list:
        score_1gram, score_2gram = RS4RE.main(i, ref_re2st_list[batch_n])
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True, prev_embedding=None, prev_encoded_layers=None, mask_qkv=None, seg_ids=None, knowledge_injection=None, concept_relation=None):
        # history embedding and encoded layer must be simultanously given
        assert (prev_embedding is None) == (prev_encoded_layers is None)

        all_encoder_layers = []
        if (prev_embedding is not None) and (prev_encoded_layers is not None):
            bertlayer_num_prev = 0
            history_states = prev_embedding
            for i, layer_module in enumerate(self.layer):
                bertlayer_num_prev += 1
            ...
            for layer_module in self.layer:
                ...
                '''knowledge injection'''
                if bertlayer_num == 1 or bertlayer_num == 2 or bertlayer_num == 4:
                # if bertlayer_num == 1:
                    if knowledge_injection != None:
                        hidden_states = knowledge_injection(sentences=concept_relation, unilm_outputs=hidden_states,
                                                            unilm_mask=attention_mask,layer=bertlayer_num)
                ''''''
                if output_all_encoded_layers:
                    all_encoder_layers.append(hidden_states)
                # if bertlayer_num == 10:
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True, mask_qkv=None, task_idx=None, knowledge_injection=None, concept_relation=None,concept_rel_attention_mask=None):
        ...
        encoded_layers = self.encoder(embedding_output, extended_attention_mask,
                                      output_all_encoded_layers=output_all_encoded_layers, mask_qkv=mask_qkv, seg_ids=token_type_ids, knowledge_injection= knowledge_injection, concept_relation=concept_relation)
...
class BertModelIncr(BertModel):
    ...
    def forward(self, input_ids, token_type_ids, position_ids, attention_mask, output_all_encoded_layers=True, prev_embedding=None,
                prev_encoded_layers=None, mask_qkv=None, task_idx=None, knowledge_injection=None, concept_relation=None):
    ...
    #             prev_embedding=None, prev_encoded_layers=None, mask_qkv=None, task_idx=None):
        ...
        encoded_layers = self.encoder(embedding_output,
                                      ...
                                      prev_embedding=prev_embedding,
                                      prev_encoded_layers=prev_encoded_layers, mask_qkv=mask_qkv, seg_ids=token_type_ids,
                                      knowledge_injection=knowledge_injection, concept_relation=concept_relation)
        sequence_output = encoded_layers[-1]
        pooled_output = self.pooler(sequence_output)
        if not output_all_encoded_layers:
            encoded_layers = encoded_layers[-1]
...
class BertForPreTrainingLossMask(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
                ...
                num_tokens_a=None, num_tokens_b=None, mask_qkv=None, knowledge_injection=None,
                concept_relation=None, copy_labels=None):
        ...
        sequence_output, pooled_output = self.bert(
            ...
            task_idx=task_idx, knowledge_injection=knowledge_injection,concept_relation=concept_relation)
        ...
        def compute_copy_loss(predictions, token_type_id, labels):
            ...
            # target_mask = token_type_id.contiguous()
            """
        ...
        # copy_predictions = self.decoder(x)
        # print('copy_predictions', copy_predictions.shape)
        copy_predictions = self.linear(nn.GELU()(sequence_output))
        # print('copy_predictions',copy_predictions.shape)
        copy_loss = compute_copy_loss(copy_predictions, token_type_ids, copy_labels)
        ...
        def compute_seq2seq_loss(predictions, token_type_id, input_ids, vocab_size):
            ...
            # 负loss
            y_pred = torch.topk(predictions, k=10)[0]
            neg_loss = torch.logsumexp(y_pred, dim=-1)

            loss = neg_loss - pos_loss
            return (loss * target_mask).sum() / target_mask.sum()  ## 通过mask 取消 pad 和句子a部分预测的影响

        # seq2seq_loss = compute_seq2seq_loss(sequence_output, token_type_ids, input_ids,
        #                                     self.config.vocab_size)

        copy_total_loss = copy_loss
        ...
        if masked_lm_labels is None:
            ...
            return prediction_scores, seq_relationship_score, copy_total_loss
        ...
        if pair_x is None or pair_y is None or pair_r is None or pair_pos_neg_mask is None or pair_loss_mask is None:
            return masked_lm_loss, next_sentence_loss, copy_total_loss
        ...
        return masked_lm_loss, next_sentence_loss, pair_loss, copy_total_loss
...
class BertForSeq2SeqDecoder(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None, mask_qkv=None,
                knowledge_injection=None, concept_relation=None, tokenizer=None):
    # def forward(self, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None, mask_qkv=None):
        if self.search_beam_size > 1:
            return self.beam_search(input_ids, token_type_ids, position_ids, attention_mask, task_idx=task_idx,
                                    mask_qkv=mask_qkv, knowledge_injection=knowledge_injection,
                                    concept_relation=concept_relation, tokenizer=tokenizer)
        ...
        while next_pos < output_length:
            ...
            new_embedding, new_encoded_layers, _ = \
                self.bert(x_input_ids, curr_token_type_ids, curr_position_ids, curr_attention_mask,
                          output_all_encoded_layers=True, prev_embedding=prev_embedding, prev_encoded_layers=prev_encoded_layers, mask_qkv=mask_qkv, knowledge_injection=knowledge_injection, concept_relation=concept_relation)
    ...
    def copy_predict(self, inputs, output_ids, states, seq2seq_predictions, copy_predictions):
        token_ids = inputs
        token_ids = torch.tensor(token_ids)

        self.linear = nn.Linear(self.config.hidden_size, 3).to(token_ids.device)
        copy_predictions = self.linear(nn.GELU()(copy_predictions))

        token_ids_new = []

        output_ids = torch.tensor([item.cpu().detach().numpy() for item in output_ids]).to(token_ids.device)
        output_ids_new = None
        for b in output_ids:
            if output_ids_new == None:
                output_ids_new = b.unsqueeze(-1)
            else:
                output_ids_new = torch.cat([output_ids_new,b.unsqueeze(-1)],dim=-1)

        for i in range(output_ids_new.shape[0]):
            for j in range(output_ids_new.shape[1]):
                token_ids_new.append(torch.cat([token_ids[i],output_ids_new[i][j]]))

        if len(output_ids) == 1:
            output_ids = output_ids[0].reshape(-1,1)
        else:
            tmp_output_ids = None
            for i in range(len(output_ids)):
                if tmp_output_ids == None:
                    tmp_output_ids = output_ids[0].view(-1,1)
                else:
                    tmp_output_ids = torch.cat((tmp_output_ids,output_ids[i].view(-1,1)),dim=-1)
            output_ids = tmp_output_ids

        # output_ids = output_ids.reshape(-1)
        # print('copy_predictions',copy_predictions.shape)
        prediction = [seq2seq_predictions[:, -1].cpu().numpy(), torch.softmax(copy_predictions[:, -1], dim=-1).cpu().numpy()]
        # 返回最后一个字符的预测结果，（batch size， vocab_size）,(batch size, 3) todo 我这里需要加一个softmax 前面的生成模型给也需要

        # print('prediction[1]',prediction[1].shape)
        # print('prediction[0]', prediction[0].shape)
        # print('token_ids_new',len(token_ids_new))

        # states用来缓存ngram的n值
        if states is None:
            states = [0] * len(token_ids_new)
        elif len(states) == 1 and len(token_ids_new) > 1:
            states = states * len(token_ids_new)
        # 根据copy标签来调整概率分布
        probas = np.zeros_like(prediction[0]) - 1000  # 最终要返回的概率分布

        output_ids = output_ids.cpu().numpy()
        for i, token_ids in enumerate(token_ids_new):
            if states[i] == 0:
        ...
        # print(states)
        return torch.tensor(probas).unsqueeze(1).cuda(), states
    ...
    def beam_search(self, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None, mask_qkv=None,
        ...
        while next_pos < output_length:
            ...
            if step_ids == []:
                prediction_scores_cp = None
                pass
            else:
                copy_predictions = last_hidden
                prediction_scores_cp, states = self.copy_predict(
                    input_ids, step_ids, states, prediction_scores, copy_predictions
                )  # 计算当前得分

            # log_scores = prediction_scores
        ...
        for b in range(batch_size):
            ...
            '''
            add lexical constrained and structure constrained
            '''
            s_list = {}
                ...
                for i, wid in enumerate(wids_list[fid]):
                    if wid == self.eos_id or fid == last_frame_id:
                        ...
                        if s > max_score:
                            ...
                            pos_in_frame = i
                        s_list[s] = [fid, i]
            s_list = sorted(s_list.items(), key=lambda x: x[0], reverse=False)
            input_i = input_ids[b]
            s_n = 0
            cover_score = 0
            total_score = {}
            for s_l in s_list:
                ...
                if frame_id == -1:
                    ...
                    cover_score = cover_num / len(input_i)
                    # ts = cover_score * 0.2 + s_l[0] * 0.8
                    ts = s_l[0]
                    total_score[ts] = seq
            '''
            structure constrained
            '''
            # cand_req = cand_dict_to_list(total_score)
            # req_structurize(tokenizer,cand_req)
            #
            # curr_req_id = b
            # stru_score_list = req_stru_eval(curr_req_id)
            # print(stru_score_list)
            #
            # n = 0
            # n_total_score = {}
            # for key, value in total_score.items():
            #     stru_score_list[n] += key
            #     n_total_score[key] = value
            #     n+=1


            total_score = sorted(total_score.items(), key=lambda x: x[0], reverse=True)
            traces['pred_seq'].append(total_score[0][1])
