...
file_path = '/root/Desktop/uav_req_gen_unilm_v2/unilm/src/crf_project/gannt'
...
def get_agent(inPath, outPath):
	...
	for i, line in enumerate(rl):
		if line != "\n":
			...
			item[":"] = item[":"][0:-1]
			if len(item["agent"]) > 1:
				item["agent"] = item["agent"][0:-1].replace(" 's", "'s")
			res.append(item)
			item = {
				"#": "",
				":": "",
		...
		if "when " in line[":"].lower() or "if " in line[":"].lower():
			...
			while j < len(rl_agent):
				...
				if agent_line != "\n":
					...
					if "execution" in output_line:
						print(output_line)
						print("here is output execution")
...
def check(prePath, outPath):
	...
	for i, item in enumerate(f_pre):
		...
		if "operation" in item:
			...
			elif one["operation"] != "" and f_out[i]["operation"] != "" and not is_substr(f_out[i]["operation"], one[
				"operation"]) and not is_substr(one["operation"], f_out[i]["operation"]):
		...
		if "input" in item:
			input_num += 1
			one["input"] = [format_sent(pp["entity"]) for pp in item["input"]["()"]]
			f_out[i]["input"] = [format_sent(pp) for pp in f_out[i]["input"]]
			print("input : ")
			print(one["input"])
			print(f_out[i]["input"])
			if sorted(one["input"]) != sorted(f_out[i]["input"]) and not is_sublist(one["input"], f_out[i][
				"input"]) and not is_sublist(f_out[i]["input"], one["input"]):
				input_error += 1
				flag += 1
				if "be" in one[':']:
					pass_input += 1
		...
		output_flag = output_error

		if "output" in f_out[i]:
			print(f_out[i]["output"])
		if "output" in item:
			output_num += 1
			one["output"] = [format_sent(pp["entity"]) for pp in item["output"]["()"]]
			f_out[i]["output"] = [format_sent(pp) for pp in f_out[i]["output"]]
			print("output: ")
			print(one["output"])
			print(f_out[i]["output"])
			if sorted(one["output"]) != sorted(f_out[i]["output"]) and not is_sublist(one["output"], f_out[i][
				"output"]) and not is_sublist(f_out[i]["output"], one["output"]):
				output_error += 1
				flag += 1
				print("output wrong")
		...
		if "restriction" in item:
			...
			restriction_num += 1
			one["restriction"] = [format_sent(res_item) for res_item in item["restriction"]["()"]]
			print("restriction: ")
			print(one["restriction"])
			if "restriction" not in f_out[i]:
				f_out[i]["restriction"] = []
			print(f_out[i]["restriction"])
			if sorted(one["restriction"]) != sorted(f_out[i]["restriction"]) and not is_sublist(one["restriction"],
																								f_out[i][
																									"restriction"]) and not is_sublist(
					f_out[i]["restriction"], one["restriction"]):
				restriction_error += 1
				flag += 1
		...
		if "event" in item:
			...
			if one['event'] != [] and "event" in f_out[i]:
				...
				print(f_item["agent"])
				if event_item["agent"].lower() not in f_item["agent"].lower() and f_item["agent"].lower() not in \
						event_item["agent"].lower():
					event_flag += 1
				...
				if (event_item["operation"] == "" and f_item["operation"] != "") or (
				...
				elif event_item["operation"] != "" and f_item["operation"] != "" and not is_substr(f_item["operation"],
																								   event_item[
																									   "operation"]) and not is_substr(
						event_item["operation"], f_item["operation"]):
					...
					if len(test1_sent) > len(test2_sent):
						for p in range(len(test2_sent)):
							test1 = test1_sent[p].lemma
							test2 = test2_sent[p].lemma
							if test1 != test2 and test2 not in event_item["operation"] and test2[0:-1] not in \
									event_item["operation"]:
								event_flag += 1
								print("event operation wrong")
								break
					else:
						for p in range(len(test1_sent)):
							...
							if test1 != test2 and test1 not in f_item["operation"] and test1[0:-1] not in f_item[
								"operation"]:
								event_flag += 1
								print("event operation wrong")
								break
				...
				# input
				event_item["input"] = [format_sent(pp) for pp in event_item["input"]]
				f_item["input"] = [format_sent(pp) for pp in f_item["input"]]
				print("event input:")
				...
				# output
				f_item["output"] = [format_sent(pp) for pp in f_item["output"]]
				print("event output:")
				...
				# restriction
				event_item["restriction"] = [format_sent(one_res) for one_res in event_item["restriction"]]
				f_item["restriction"] = [format_sent(one_res) for one_res in f_item["restriction"]]
	...
	final_json_list = []
	try:
		with open('/Users/tuanz_lu/PycharmProjects/Pytorch-learning/gannt/tuple_eval.json', 'r') as f:
	...
	final = open('/Users/tuanz_lu/PycharmProjects/Pytorch-learning/gannt/tuple_eval.json', 'w')
			...
			for sent in doc.sentences:
				for j, word in enumerate(sent.words):
					if res_flag == 1:
						...
						cur_restriction = ""
					elif word.deprel == "case" or word.deprel == "mark" or word.text == "maximum" or (
							word.xpos == "DT" and word.deprel == "advmod"):
	...
	for j, cur_temp in enumerate(temp):
		for p, cur_c in enumerate(c_list):
			for tag in tag_list:
				...
				print(cur_tag)
				agent_path = file_path + '/structure/gannt/agent_' + cur_tag + '.output'
				operation_path = file_path + '/structure/gannt/operation_' + cur_tag + '.output'
				input_path = file_path + '/structure/gannt/input_' + cur_tag + '.output'
				output_path = file_path + '/structure/gannt/output_' + cur_tag + '.output'
				restriction_path = file_path + '/structure/gannt/restriction_' + cur_tag + '.output'
				event_agent_path = file_path + '/structure/gannt/event_agent_' + cur_tag + '.output'
				event_operation_path = file_path + '/structure/gannt/event_operation_' + cur_tag + '.output'
				event_input_path = file_path + '/structure/gannt/event_input_' + cur_tag + '.output'
				event_output_path = file_path + '/structure/gannt/event_output_' + cur_tag + '.output'
				event_restriction_path = file_path + '/structure/gannt/event_restriction_' + cur_tag + '.output'

				get_agent(inPath=agent_path, outPath=file_path + '/external/test_' + tag + '.json')
				get_operation(inPath=operation_path, outPath=file_path + '/external/test_' + tag + '.json')
				get_input(inPath=input_path, outPath=file_path + '/external/test_' + tag + '.json')
				get_output(inPath=output_path, outPath=file_path + '/external/test_' + tag + '.json')
				get_restriction(inPath=restriction_path, outPath=file_path + '/external/test_'+ tag + '.json')

				get_event(event_agent_path, event_operation_path, event_input_path, event_output_path,
						  event_restriction_path,
						  outPath=file_path + '/external/test_' + tag + '.json')
				test_restriction(outPath=file_path + '/external/test_' + tag + '.json')

				# final_item = check(
				# 	prePath='/root/Desktop/crf_project/crf_project/json_data/' + tag.upper() + '.json',
				# 	outPath=file_path + '/external/test_' + tag + '.json')
