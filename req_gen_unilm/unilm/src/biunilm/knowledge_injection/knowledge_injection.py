...
class Encoder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # lstm_output, (h, c) = self.knowledge_encoder(inputs)
...
def filter_n_hop_rel_n_layer(sen, layer=0):
    rel_list = sen.strip().split('.')
    filter_result = []
    for rel in rel_list:
        pattern = '#(.+?)#'
        ...
        if len(hop) > 0:
            ...
            hop_value = 1
        if layer == 2 and (hop_value == 1 or hop_value == 2):
            filter_result.append(rel)
        if layer == 4 and (hop_value == 1):
            filter_result.append(rel)
        # if layer == 8 and (hop_value == 1):
        #     filter_result.append(rel)
        # if layer == 11 and (hop_value ==1):
        #     filter_result.append(rel)
        # if hop_value == 1:
        #     filter_result.append(rel)
        # elif hop_value == 2 and layer <= 1:
        #     filter_result.append(rel)
        # elif hop_value == 3 and layer <= 1:
        #     filter_result.append(rel)
        # elif hop_value == 4 and layer <= 1:
        #     filter_result.append(rel)
    filter_sen = '.'.join(filter_result)
    return filter_sen
...
class knowledge_injection(nn.Module):
    ...
    def forward(self, sentences, unilm_outputs, unilm_mask, layer=0):
        device = torch.device(
            "cuda" if torch.cuda.is_available() else "cpu")
        ...
        for sen in sentences:
            sen = filter_n_hop_rel_rand(sen)
            if layer == 1:
                pass
            else:
                sen = filter_n_hop_rel_n_layer(sen, layer)
            pattern = '#(.+?)#'
