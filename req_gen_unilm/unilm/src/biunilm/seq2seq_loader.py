...
class Seq2SeqDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, file_src, file_tgt, file_rel, batch_size, tokenizer, max_len, file_oracle=None, short_sampling_prob=0.1, sent_reverse_order=False, bi_uni_pipeline=[]):
        super().__init__()
        print('change')
        ...
        if file_oracle is None:
            with open(file_src, "r", encoding='utf-8') as f_src, open(file_tgt, "r", encoding='utf-8') as f_tgt, open(file_rel, "r", encoding='utf-8') as f_rel:
                rel_list = []
                for rel in f_rel:
                    rel_list.append(rel)
                for src, tgt, rel in zip(f_src, f_tgt, rel_list):
                    src_tk = tokenizer.tokenize(src.strip())
                    tgt_tk = tokenizer.tokenize(tgt.strip())
                    assert len(src_tk) > 0
                    assert len(tgt_tk) > 0
                    self.ex_list.append((src_tk, tgt_tk, rel_list.index(rel)))
    ...
    def __getitem__(self, idx):
        instance = self.ex_list[idx][:-1]
        index = self.ex_list[idx][-1]
...
class Preprocess4Seq2seq(Pipeline):
    ...
    def __call__(self, instance):
        ...
        labels = source_labels[1:] + target_labels  # 因为是预测所以第一位后移
        ...
        return (input_ids, segment_ids, input_mask, mask_qkv, masked_ids, masked_pos, masked_weights, -1, self.task_idx, labels)
